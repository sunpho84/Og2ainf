...
void intComp(vd_t& Int, vd_t& Inta0,vd_t& IntS,vd_t& IntSa0,int i)
{
  ...
  Matrix4d A;
    ...
    /* Vertex Integrals */
    double of4den=1/(4.*Den);
    double of8den=1/(8.*Den);
    double of16den=1/(16.*Den);
    
    Int[0]  += Fac*(-CPxSxSSP*CPxSxSSP*of4den + SSP2/(4.*Delta1*Delta2p*Delta2p));
    Int[1]  += Fac*(CPxSxSSP*CPxSxSSP*of4den);
    Int[2]  += Fac*(-CPxSxSSPxAtxCPxSxSSP*of4den+(CPxSPxSSPxAtxS2*Delta1p)/(Den)-(CPxSxSSPxAtxSxSP*Delta1p)/(Den)+(CPxSPxSSP*Delta1p)/(Delta1*Delta2p*Delta2p)+(Delta1p*Delta1p*SP2)/(Delta1*Delta2p*Delta2p)+(Delta1p*Delta1p*SP2xAtxS2)/(Den)-SP2xSSP2/(4.*Delta1*Delta2p*Delta2p)-SP2xSSP2xAtxS2*of4den+SSP2xAtxS2*of4den-(CPxSxSSP*Delta1p*SxSP)/(Den)-(Delta1p*Delta1p*SxSP*SxSP)/(Den)-(Delta1p*Delta1p*SxSPxAtxSxSP)/(Den));
    Int[3]  += Fac*((CPxSxSSP*Delta1p*SxSP)/(Den)+(Delta1p*Delta1p*SxSP*SxSP)/(Den));
    Int[4]  += Fac*((CPxSPxSSPxAtxS2*Delta1p)*of4den-(CPxSxSSPxAtxSxSP*Delta1p)*of4den+(CPxSPxSSP*Delta1p)/(4.*Delta1*Delta2p*Delta2p)-(AtxS2*Delta1p*Delta1p)*of4den-(3*Delta1p*Delta1p)/(4.*Delta1*Delta2p*Delta2p)-(Delta1p*Delta1p*S2xSP2)*of4den+(Delta1p*Delta1p*SP2)/(4.*Delta1*Delta2p*Delta2p)+(Delta1p*Delta1p*SP2xAtxS2)*of4den-(SP2*SSP2)/(16.*Delta1*Delta2p*Delta2p)-(SP2xAtxS2*SSP2)*of16den-(CPxSxSSP*Delta1p*SxSP)*of4den+(SSP2*SxSP*SxSP)*of16den+(SSP2*SxSPxAtxSxSP)*of16den);
    Int[5]  += Fac*(-Delta1p*Delta1p/(4.*Delta1*Delta2p*Delta2p) + (Delta1p*Delta1p*S2xSP2)*of4den + (CPxSxSSP*Delta1p*SxSP)*of4den - (SSP2*SxSP*SxSP)*of16den);
    Int[6]  += Fac*(CPxSxSSPxAtxCPxSxSSP*of4den+SP2xSSP2/(4.*Delta1*Delta2p*Delta2p)+SP2xSSP2xAtxS2*of4den+(AtxS2*SSP2)*of4den+(S2xSP2*SSP2)*of4den-(SP2*SSP2)/(4.*Delta1*Delta2p*Delta2p)-(SP2xAtxS2*SSP2)*of4den-SSP2xAtxS2*of4den);
    Int[7]  += Fac*(-(S2xSP2*SSP2)*of4den);
    Int[8]  += Fac*((CPxSxSSxAtxSSxSSP*Delta1p)*of16den-(CxCPxSSxSSPxAtxS2*Delta1p)*of8den-(CxCPxSSxSSP*Delta1p)/(8.*Delta1*Delta2p*Delta2p)+(CxCPxAtxS2*Delta1p*SSxSSP)*of8den+(CxCP*Delta1p*SSxSSP)/(8.*Delta1*Delta2p*Delta2p));
    ...
    Int[10] += Fac*(-(S2xSSP2*SS2)/(16.*Delta1*Delta2p*Delta2p)-(S2xSSP2xAtxS2*SS2)*of16den-(S2xSS2*SSP2)/(16.*Delta1*Delta2p*Delta2p)-(S2xSS2xAtxS2*SSP2)*of16den+(SS2*SSP2)/(16.*Delta2p*Delta2p)-(AtxS2*SS2*SSP2)*of16den-(SS2*SSP2)/(8.*Delta1*Delta2p*Delta2p)+(S2xAtxS2*SS2*SSP2)*of16den+(SS2xAtxS2*SSP2)*of16den-(SS2xAtxSS2*SSP2)/(64.*Den)+(SS2*SSP2xAtxS2)*of16den+(S2xSSxSSP*SSxSSP)/(8.*Delta1*Delta2p*Delta2p)+(S2xSSxSSPxAtxS2*SSxSSP)*of8den-SSxSSP*SSxSSP/(16.*Delta2p*Delta2p)+(AtxS2*SSxSSP*SSxSSP)*of16den+SSxSSP*SSxSSP/(8.*Delta1*Delta2p*Delta2p)-(S2xAtxS2*SSxSSP*SSxSSP)*of16den-(SSxSSP*SSxSSPxAtxS2)*of8den+(SSxSSP*SSxSSPxAtxSS2)/(32.*Den)-(SS2*SSxSSPxAtxSSxSSP)/(64.*Den));
    Int[11] += Fac*((Delta1p*Delta1p*S2xSS2)/(4.*Delta1*Delta2p*Delta2p)+(Delta1p*Delta1p*S2xSS2xAtxS2)*of4den-(Delta1p*Delta1p*SS2)/(4.*Delta2p*Delta2p)+(AtxS2*Delta1p*Delta1p*SS2)*of4den+(3*Delta1p*Delta1p*SS2)/(4.*Delta1*Delta2p*Delta2p)-(Delta1p*Delta1p*S2xAtxS2*SS2)*of4den-(Delta1p*Delta1p*SS2xAtxS2)*of4den+(Delta1p*Delta1p*SS2xAtxSS2)*of16den);
